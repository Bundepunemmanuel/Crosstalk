...
export default function Admin({ user }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Merge conversion counts into profiles
      ...
      const visitStats = await getPageVisitStats()
      setStats(visitStats)
    } catch (err) {
  ...
  const formatDate = (iso) => iso ? new Date(iso).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: '2-digit' }) : '—'
  ...
  return (
    <div className="min-h-screen bg-dark-900 bg-mesh">
      ...
      <main className="max-w-6xl mx-auto px-4 pt-28 pb-16">
        ...
        {actionMsg && (
          <div className="mb-6 px-4 py-3 rounded-xl bg-violet-500/10 border border-violet-500/20 text-violet-300 text-sm">{actionMsg}</div>
        )}
        ...
        <div className="card">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
            <h2 className="font-display font-semibold text-white text-lg">Users ({users.length})</h2>
            <input
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search by email..."
              className="input-field text-sm py-2 px-3 sm:w-64"
            />
          </div>
              ...
              <thead>
                <tr className="border-b border-white/5">
                  {['Email', 'Status', 'Credits', 'Conversions', 'Joined', 'Actions'].map(h => (
                    <th key={h} className="text-left text-white/30 text-xs font-mono uppercase tracking-wider pb-3 pr-4 whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {filteredUsers.map(u => (
                  <tr key={u.id} className="hover:bg-white/2 transition-colors">
                    ...
                    <td className="py-3 pr-4">
                      <span className="text-white/50 text-xs font-mono">{u.conversion_count || 0}</span>
                    </td>
                    <td className="py-3 pr-4">
                      <span className="text-white/30 text-xs font-mono whitespace-nowrap">{formatDate(u.created_at)}</span>
                    </td>
                    <td className="py-3">
                      <div className="flex items-center gap-2 flex-wrap">
                        <div className="flex items-center gap-1">
                          <input
                            ...
                            className="w-14 bg-dark-700 border border-white/10 rounded-lg px-2 py-1 text-xs text-white font-mono focus:outline-none focus:border-violet-500/50"
                          />
                          <button
                            onClick={() => addCredits(u.id, u.email)}
                            className="text-xs px-2 py-1 rounded-lg bg-violet-500/20 border border-violet-500/30 text-violet-300 hover:bg-violet-500/30 transition-colors whitespace-nowrap"
                          >
                            +Credits
                          </button>
                        </div>
